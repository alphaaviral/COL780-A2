...
def calculateHessian(image, patch):
    ...
    # print(time.time() - start_time)
    print("Calc hessian using scipy")
    start_time = time.time()
    ...
    print(time.time() - start_time)
    # newHessian = np.zeros((image.shape[0], image.shape[1], 2, 2))
    # for i in range(image.shape[0]):
    #     for j in range(image.shape[1]):
    #         newHessian[i][j][0][0] = Ixx[i][j]
    #         newHessian[i][j][1][1] = Iyy[i][j]
    #         newHessian[i][j][1][0] = Ixy[i][j]
    #         newHessian[i][j][0][1] = Ixy[i][j]
    
    # print(np.array_equal(hessian,newHessian))
